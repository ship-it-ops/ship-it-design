/**
 * Token resolution helpers shared by Ship-It graph adapters. The design system
 * stores colors as CSS custom properties on `<html>` (`--color-accent`,
 * `--color-bg`, …). Graph engines like Cytoscape and React Flow render to a
 * canvas / SVG layer outside Tailwind, so those vars never resolve — we read
 * the *computed* values at runtime and feed them into the engine as concrete
 * color strings.
 *
 * Two layers:
 *   1. `resolveCssVar` — single-var reader with a fallback.
 *   2. `readThemeTokens` — pulls every color the graph adapters use, returning
 *      a flat object keyed by short name (no `--color-` prefix). Re-run after
 *      a `data-theme` flip to pick up the new palette.
 */
...
// non-DOM color parsers accept. Created on first use; the same canvas is
 ...
 * console warnings and silently fall back to defaults on every mount.
 * Coercing once at the boundary fixes both the warnings and the fallback
 * colors. The same constraint applies to React Flow's CSS-variable bridge.
 *
 ...
 * `--color-*` token to `oklch(...)`, which non-DOM color parsers reject — see
 * {@link toSrgb} for the full reasoning.
 */
