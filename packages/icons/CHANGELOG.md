...
- 035d1c5: Add a rocket glyph variety pack: canonical `rocket` (Lucide) plus 24 variants
  across Phosphor (`rocketAlt`/`rocketThin`/`rocketLight`/`rocketBold`/`rocketFill`/
  `rocketDuotone` and the diagonal `rocketLaunch` family in the same six weights)
  and Solar (`rocket*` and tilted `rocket2*` families in Linear, Outline, Broken,
  Solid, SolidDuotone, and LineDuotone styles). Also add the Rocket.rs framework
  logo as `rocket` in the logo set.

## 0.0.14

### Patch Changes

- 52af924: Add automotive + premium glyphs and solid icon variants, and harden the
...
- 52af924: Rename the `connector` icon category to `logo`. The brand-mark set (GitHub,
...
- d4e29e1: Add social-media and tech brand glyphs to the icon manifest.
