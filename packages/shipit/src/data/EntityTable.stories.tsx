...
const services: Service[] = [
  { id: 's1', type: 'service', name: 'payment-webhook-v2', owner: 'Payments', runtime: 'node 20', deploys: 2.3, status: 'ok' },
  { id: 's2', type: 'service', name: 'ledger-core', owner: 'Payments', runtime: 'go 1.22', deploys: 0.8, status: 'ok' },
  { id: 's3', type: 'service', name: 'notify-dispatch', owner: 'Platform', runtime: 'node 20', deploys: 4.1, status: 'warn' },
  { id: 's4', type: 'service', name: 'auth-edge', owner: 'Platform', runtime: 'rust', deploys: 0.2, status: 'ok' },
  { id: 's5', type: 'service', name: 'legacy-wh', owner: 'Payments', runtime: 'node 16', deploys: 0, status: 'off' },
];
...
const columns: DataTableColumn<Service>[] = [
  ...
  { key: 'runtime', header: 'Runtime', accessor: (r) => r.runtime, cell: (r) => <span className="font-mono text-text-muted">{r.runtime}</span> },
  {
    ...
    align: 'right',
    cell: (r) => <span className="font-mono tabular-nums">{r.deploys === 0 ? '—' : r.deploys.toFixed(1)}</span>,
  },
