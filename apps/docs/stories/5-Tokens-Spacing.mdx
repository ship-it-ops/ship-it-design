...
export const Bar = ({ token, value }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '6px 0' }}>
    <code style={{ fontFamily: 'var(--font-family-mono)', fontSize: 11, color: 'var(--color-text-dim)', minWidth: 70 }}>space-{token}</code>
    <code style={{ fontFamily: 'var(--font-family-mono)', fontSize: 11, color: 'var(--color-text)', minWidth: 50 }}>{value}</code>
