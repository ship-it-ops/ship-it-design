...
export const SwatchRow = ({ tokens }) => (
  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
    {tokens.map(([name, css]) => (
      <div key={name} style={{ border: '1px solid var(--color-border)', borderRadius: 8, overflow: 'hidden', background: 'var(--color-panel)' }}>
        <div style={{ height: 64, background: css, borderBottom: '1px solid var(--color-border)' }} />
...
:root { --accent-h: 280; }   /* purple */
:root { --accent-h: 150; }   /* green  */
:root { --accent-h: 25; }    /* coral  */
```
