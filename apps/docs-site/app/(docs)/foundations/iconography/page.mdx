import { IconGlyph, logoManifest, glyphManifest } from '@ship-it-ui/icons';
...
export const logoItems = Object.keys(logoManifest)
  .sort()
  .map((name) => ({ name, kind: 'logo' }));

<IconCatalog items={logoItems} pageSize={64} searchPlaceholder="Search logos…" columns={5} />
