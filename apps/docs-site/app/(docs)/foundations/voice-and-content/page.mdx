...
None. We render iconography through `<IconGlyph>` — inline SVGs sourced from
Iconify (Lucide for UI primitives, Phosphor for soft variants, simple-icons
for brand logos). The brand mark itself is the same pipeline: `<IconGlyph name="brand" />`
(currently Lucide's `rocket` — Ship-It is a deployment story).
