...
- **`label: ReactNode`** (required) — bucket name shown on the left of
  the row (e.g. "Errors", "Latency", "Saturation").
- **`value: number`** (required) — the bucket's contribution as a
  number. Rendered in the row's value column.
- **`tone?: 'accent' | 'ok' | 'warn' | 'err'`** — tone for the value
  text. Defaults to inheriting the parent score's tone, so an `ok`
  overall score gets `ok` rows by default.
