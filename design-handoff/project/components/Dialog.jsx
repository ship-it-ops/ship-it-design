    ...
    <>
      ...
      <div role="dialog" aria-modal="true" style={{
        ...
        <div style={{
          ...
          borderRadius: 12, boxShadow: tokens.shadowLg,
          padding: 24, pointerEvents: 'auto',
          animation: 'dialogIn 180ms cubic-bezier(0.2,0.8,0.2,1)',
        }}>
          {title && (
            <div style={{ fontSize: 16, fontWeight: 500, marginBottom: description ? 6 : 16 }}>{title}</div>
          )}
          {description && (
            <div style={{ fontSize: 13, color: tokens.textMuted, marginBottom: 18, lineHeight: 1.55 }}>{description}</div>
          )}
          {children}
          {footer && (
            <div style={{ marginTop: 20, display: 'flex', gap: 8, justifyContent: 'flex-end' }}>{footer}</div>
          )}
        </div>
      </div>
    </>
...
function Drawer({ open, onClose, side = 'right', title, children, width = 420 }) {
  ...
  return (
    <>
      <Backdrop onClick={onClose}/>
      <div style={{
        position: 'fixed', top: 0, bottom: 0, [side]: 0, zIndex: 51,
        width, background: tokens.panel,
