...
function Card({ title, description, footer, actions, children, onClick, hover, style = {} }) {
  ...
  return (
    ...
    >
      {(title || actions) && (
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: description || children ? 10 : 0 }}>
          {title && <div style={{ fontSize: 14, fontWeight: 500, flex: 1 }}>{title}</div>}
          {actions && <div style={{ display: 'flex', gap: 4 }}>{actions}</div>}
        </div>
      )}
      {description && (
        <div style={{ fontSize: 12, color: tokens.textMuted, lineHeight: 1.55, marginBottom: children ? 14 : 0 }}>{description}</div>
      )}
      {children}
      {footer && (
        <div style={{ marginTop: 14, paddingTop: 12, borderTop: `1px solid ${tokens.border}`, fontSize: 11, color: tokens.textDim }}>{footer}</div>
      )}
    </div>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        <div style={{ fontFamily: tokens.fontMono, fontSize: 10, color: tokens.textDim, textTransform: 'uppercase', letterSpacing: 1.4 }}>{label}</div>
        {icon && <div style={{ color: tokens.textDim, fontSize: 14 }}>{icon}</div>}
      </div>
      <div style={{ fontSize: 28, fontWeight: 300, letterSpacing: -0.8, color: tokens.text, lineHeight: 1 }}>{value}</div>
      {delta && (
