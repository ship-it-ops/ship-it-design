...
function ToastCard({ toast, onDismiss }) {
  ...
  return (
    <div style={{
      minWidth: 280, maxWidth: 380, padding: 12,
      background: v.bg, border: `1px solid ${v.border}`,
      borderRadius: 8, boxShadow: tokens.shadowLg,
      ...
      <div style={{
        ...
        color: v.iconColor, display: 'grid', placeItems: 'center',
        fontSize: 11, fontWeight: 600, flexShrink: 0, marginTop: 1,
      }}>{v.icon}</div>
      <div style={{ flex: 1, minWidth: 0 }}>
        {toast.title && <div style={{ fontSize: 13, fontWeight: 500 }}>{toast.title}</div>}
        {toast.description && <div style={{ fontSize: 12, color: tokens.textMuted, marginTop: 2, lineHeight: 1.5 }}>{toast.description}</div>}
        {toast.action && (
          <div style={{ marginTop: 8 }}>{toast.action}</div>
        )}
      </div>
      <button onClick={onDismiss} style={{
        background: 'transparent', border: 'none', color: tokens.textDim,
        cursor: 'pointer', fontSize: 15, padding: 0, lineHeight: 1, marginTop: -2,
      }}>×</button>
    </div>
  );
}
