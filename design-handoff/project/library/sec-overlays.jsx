...
function SecOverlays() {
  return (
    <Section
      ...
      title="Overlays"
      count="9 patterns"
      desc="Click any trigger to open the real overlay. All are dismissable with ESC or backdrop click."
    >
      <Subsection title="Dialog (modal)" note="live">
        <Specimen
          code={`<Dialog open={open} onClose={…} title="Disconnect github">
  Are you sure?
</Dialog>`}
        >
          <LiveDialogDemo />
      ...
      <Subsection title="Tooltip" note="live · hover">
        <Specimen
          code={`<Tooltip content="Add new source">
  <IconButton icon="+"/>
</Tooltip>`}
        >
          <Row gap={20}>
            ...
            <Tooltip content="On call · Priya K.">
              <Avatar name="Priya Khanna" status="ok" />
            </Tooltip>
            <Tooltip content="3 unresolved incidents">
      ...
      <Subsection title="Dropdown menu" note="live">
        <Specimen
          code={`<DropdownMenu trigger={<Button trailing="▾">Actions</Button>}>
  ...
  <Card>Right-click me</Card>
      ...
      <Subsection title="Hover card">
        <Specimen
          code={`<HoverCard content={<EntityPreview id="…"/>}>
            ...
            {[
              ...
              <div
                ...
                style={{
                  ...
                  borderLeft: `2px solid var(--${t.v})`,
                  borderRadius: 8,
                  boxShadow: T.shadow,
                }}
              >
                <Row gap={10} style={{ alignItems: 'flex-start' }}>
                  <span style={{ color: `var(--${t.v})`, fontSize: 14, marginTop: 1 }}>
                    {t.v === 'ok' ? '✓' : t.v === 'warn' ? '!' : t.v === 'err' ? '×' : 'ℹ'}
                  </span>
                  <div style={{ flex: 1 }}>
...
function LiveDialogDemo() {
  ...
  return (
    <Row gap={10}>
      <UIBtn variant="secondary" onClick={() => setOpen(true)}>
        Open dialog
      </UIBtn>
      {open && (
        <div
        ...
        >
          ...
          >
            <div style={{ fontSize: 17, fontWeight: 500, letterSpacing: -0.3, marginBottom: 4 }}>
              Disconnect github?
            </div>
            <div style={{ fontSize: 13, color: T.textMuted, lineHeight: 1.55, marginBottom: 20 }}>
              This stops live sync immediately. Your existing graph won't change, but new commits,
              PRs, and code references won't appear.
            </div>
            <Row gap={8} style={{ justifyContent: 'flex-end' }}>
              <UIBtn variant="ghost" onClick={() => setOpen(false)}>
                Cancel
              </UIBtn>
              <UIBtn variant="danger" onClick={() => setOpen(false)}>
                Disconnect
              </UIBtn>
            </Row>
          </div>
    ...
    <>
      ...
      {open && (
        <>
          <div
            onClick={() => setOpen(false)}
            style={{ position: 'fixed', inset: 0, background: 'oklch(0 0 0 / 0.4)', zIndex: 100 }}
          />
          <div
            style={{
              position: 'fixed',
          ...
          >
            <Row style={{ marginBottom: 20 }}>
              ...
              <span
                onClick={() => setOpen(false)}
                style={{ marginLeft: 'auto', cursor: 'pointer', color: T.textDim }}
              >
                ×
              </span>
            </Row>
            ...
            <Col gap={6}>
              {[
                ...
                <Row
                  key={l}
                  gap={10}
                  onClick={k === '⌘.' ? () => setOpen(false) : undefined}
                  style={{
                    padding: '8px 12px',
                    background: T.panel2,
                    borderRadius: 6,
...
function LivePopoverDemo() {
  ...
  return (
    <div style={{ position: 'relative', display: 'inline-block' }}>
      <UIBtn variant="secondary" onClick={() => setOpen((o) => !o)} trailing="▾">
        Show popover
      </UIBtn>
      {open && (
        <div
        ...
        >
          ...
          <Row gap={6}>
            ...
            <UIBtn size="sm" variant="ghost" onClick={() => setOpen(false)}>
              Close
            </UIBtn>
          </Row>
        </div>
              ...
              it.type === 'sep' ? (
                ...
                <Row
                  ...
                  style={{
                    ...
                    borderRadius: 4,
                    cursor: 'pointer',
                    color: it.destructive ? T.err : T.text,
                    fontSize: 12,
                  }}
                >
                  <span>{it.l}</span>
