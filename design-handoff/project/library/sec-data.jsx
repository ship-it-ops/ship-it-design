...
function SecData() {
  return (
    <Section
      ...
      title="Data Display"
      count="6 patterns"
      desc="Tables, lists, trees, kanban, timeline. Click headers to sort. Drag kanban cards between columns."
    >
        ...
        <Specimen pad={22} code={`<Timeline events={…}/>`}>
          <div style={{ position: 'relative', paddingLeft: 24 }}>
            ...
            {[
              ...
              {
                t: 'Schema extracted',
                m: '142 entity types · 38 relations',
                time: '4m ago',
                color: T.ok,
              },
              {
                t: 'First query answered',
                m: 'Who owns the payment webhook?',
                time: '6m ago',
                color: T.ok,
              },
              ...
              <div key={i} style={{ marginBottom: 18, position: 'relative' }}>
                ...
                <div style={{ fontSize: 10, color: T.textDim, fontFamily: FM, marginTop: 2 }}>
                  {e.time}
                </div>
              </div>
            ))}
  ...
  return (
    ...
    >
      <Row gap={10} style={{ padding: '12px 16px', borderBottom: `1px solid ${T.border}` }}>
        ...
        <UIBtn size="sm" variant="ghost" leading="↓">
          Export
        </UIBtn>
        <UIBtn size="sm" variant="secondary" leading="+">
          New
        </UIBtn>
      </Row>
        ...
        <tbody>
          {sorted.map((r, i) => (
            ...
            >
              ...
              <td style={{ padding: '10px 12px', fontFamily: FM, color: T.textMuted }}>
                {r.runtime}
              </td>
              <td
                style={{
              ...
              >
                {r.deploys === 0 ? '—' : `${r.deploys.toFixed(1)}`}
              </td>
      ...
      >
        <span>
          Showing {sorted.length} of 142 · sorted by{' '}
  ...
  return (
    ...
    >
      {selected.size > 0 && (
        ...
        >
          <span style={{ fontSize: 11, color: T.accent, fontFamily: FM }}>
          ...
          <span
            ...
            style={{ marginLeft: 'auto', fontSize: 11, color: T.textDim, cursor: 'pointer' }}
          >
            Clear
          </span>
        </Row>
          ...
          <tr style={{ background: T.panel2, borderBottom: `1px solid ${T.border}` }}>
            <th style={{ ...gridHead(28), padding: '6px 10px' }}>
              <input
                ...
                onChange={() => setSelected(allSel ? new Set() : new Set(rows.map((r) => r[0])))}
              />
            </th>
            {['id', 'type', 'name', 'confidence', 'source', 'updated'].map((h, i) => (
              <th key={h} style={{ ...gridHead(), textAlign: i === 3 ? 'right' : 'left' }}>
                {h}
              </th>
            ))}
          </tr>
              ...
              {colCards.map((c) => (
                ...
                >
                  ...
                  <Row gap={6}>
                    <Badge
                      size="sm"
                      color={c.p === 'high' ? 'err' : c.p === 'med' ? 'warn' : 'neutral'}
                    >
                      {c.p}
                    </Badge>
                    {c.who && (
                      <div style={{ marginLeft: 'auto' }}>
                        <Avatar size="xs" initials={c.who} />
                      </div>
                    )}
                  </Row>
                </div>
