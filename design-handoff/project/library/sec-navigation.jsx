...
function SecNavigation() {
  return (
    <Section
      ...
      title="Navigation"
      count="20+"
      desc="Sidebar, topbar, tabs, breadcrumbs, pagination, command palette, menubar, stepper. Navigation stays quiet — typography weight does the work, not heavy backgrounds."
    >
                  ...
                  <Row gap={8}>
                    <div
                      style={{
                        ...
                        borderRadius: 999,
                        background:
                          state === 'done' ? T.accent : state === 'current' ? T.accentDim : T.panel,
                        border: `1px solid ${state === 'upcoming' ? T.border : T.accent}`,
                        color:
                          state === 'done' ? '#0a0a0b' : state === 'current' ? T.accent : T.textDim,
                        display: 'grid',
                        placeItems: 'center',
                        fontSize: 11,
                        fontFamily: FM,
                    ...
                    <span
                      style={{
                        fontSize: 12,
                        color: state === 'upcoming' ? T.textDim : T.text,
                  ...
                  {i < 3 && (
                    <div
                      style={{
                        flex: 1,
                        height: 1,
                        background: i < 1 ? T.accent : T.border,
                        margin: '0 12px',
                      }}
                    />
                  )}
  ...
  return (
    ...
    >
      <Row gap={8} style={{ marginBottom: 18 }}>
        ...
        <div style={{ fontFamily: FM, fontSize: 9, color: T.textDim, marginLeft: 'auto' }}>
          acme
        </div>
      </Row>
      <div
      ...
      >
        Sources <span style={{ float: 'right', color: T.textDim, cursor: 'pointer' }}>+</span>
      </div>
      <Col gap={2}>
        {['github · 4 repos', 'notion · 182 docs', 'linear · 34 issues', 'slack · 6 channels'].map(
          (s) => (
            <div
              key={s}
              style={{
...
function LiveCommandPalette() {
  ...
  const results = all
    .filter(
      (r) =>
        r.t.toLowerCase().includes(q.toLowerCase()) || r.m.toLowerCase().includes(q.toLowerCase()),
    )
    .slice(0, 4);
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: 8, minHeight: 220 }}>
        ...
        {results.length === 0 && (
          <div
            style={{ padding: '20px 10px', fontSize: 12, color: T.textDim, textAlign: 'center' }}
          >
            No matches
          </div>
        )}
        {results.map((r, i) => (
          ...
          >
            <span
              style={{ color: i === cursor ? T.accent : T.textMuted, fontFamily: FM, fontSize: 12 }}
            >
              {r.t.split(' ')[0]}
            </span>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, color: i === cursor ? T.accent : T.text }}>
                {r.t.slice(2)}
              </div>
              <div style={{ fontSize: 11, color: T.textDim }}>{r.m}</div>
            </div>
            {i === cursor && (
              <span style={{ fontFamily: FM, fontSize: 10, color: T.textDim }}>↵</span>
            )}
          </div>
        ))}
      </div>
