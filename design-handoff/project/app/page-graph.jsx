...
function PageGraph({ nav, onAsk, initialFocus = 'payments-api' }) {
  ...
  const nodeIds = Object.keys(visited).filter(
    (id) => byId[id] && selectedTypes.includes(byId[id].type),
  );
    ...
    <div style={{ display: 'flex', height: '100%' }}>
      ...
      >
        ...
        <div style={{ fontSize: 11, color: t.textDim, marginBottom: 14 }}>
          depth {depth} · {nodeIds.length} nodes · {edges.length} edges
        </div>
          ...
          {[1, 2, 3].map((d) => (
            <div
              ...
              style={{
                ...
                border: `1px solid ${depth === d ? t.accent : t.border}`,
                borderRadius: 5,
                cursor: 'pointer',
              }}
            >
        ...
        {[
          ...
          return (
            <div
              key={l}
              onClick={() =>
                setSelectedTypes(on ? selectedTypes.filter((x) => x !== l) : [...selectedTypes, l])
              }
              style={{
                display: 'flex',
                alignItems: 'center',
            ...
            >
              ...
              <div style={{ marginLeft: 'auto', fontSize: 10, color: t.textDim, fontFamily: MONO }}>
                {n}
              </div>
            </div>
          );
          ...
          >
            <div
              style={{
                ...
                borderRadius: 999,
                border: `1px solid ${t.borderStrong}`,
                background: env === e ? t.accent : 'transparent',
              }}
            />
          ...
          {['radial', 'hierarchical', 'force'].map((l) => (
            <div
              ...
              style={{
                ...
                border: `1px solid ${layout === l ? t.borderStrong : t.border}`,
                borderRadius: 5,
                cursor: 'pointer',
              }}
            >
        ...
        >
          <div>
            <span style={{ color: t.purple }}>MATCH</span> (s)-[*1..{depth}]-(n)
          </div>
          <div>
            <span style={{ color: t.purple }}>WHERE</span> s.name ={' '}
            <span style={{ color: t.accent }}>"{focus}"</span>
          </div>
          <div>
            <span style={{ color: t.purple }}>RETURN</span> n, r
          </div>
        </div>
      </div>
      ...
      <div
        style={{
          flex: 1,
          position: 'relative',
          background: `radial-gradient(circle at 50% 45%, ${t.panel2} 0%, ${t.bg} 70%)`,
          overflow: 'hidden',
        }}
      >
        {/* Subtle grid */}
        <div
          style={{
            ...
            maskImage: 'radial-gradient(circle at center, #000 30%, transparent 70%)',
          }}
        />
        ...
        >
          <div
            onClick={() => onAsk(`show everything that depends on ${focus} in ${env}`)}
            style={{
              background: t.panel,
              border: `1px solid ${t.border}`,
              borderRadius: 8,
          ...
          >
            ...
            <span style={{ color: t.text }}>
              show me everything that depends on {focus} in {env}
            </span>
            <AskHint style={{ marginLeft: 'auto' }} />
          </div>
          <Btn small style={{ padding: '8px 12px' }}>
            {layout}
          </Btn>
          <Btn small ghost style={{ padding: '8px 12px' }}>
            ⌕
          </Btn>
          <Btn small ghost style={{ padding: '8px 12px' }}>
            ⊟
        ...
        {focusNode && (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
              ...
              <Pill
                color={focusNode.tier === 1 ? t.err : focusNode.tier === 2 ? t.warn : t.textMuted}
              >
                Tier {focusNode.tier}
              </Pill>
            </div>
            <div style={{ fontSize: 22, fontWeight: 600, letterSpacing: -0.4, marginBottom: 2 }}>
              {focusNode.name}
            </div>
            ...
            >
              {[
                ...
                >
                  <div
                    style={{ fontSize: 16, fontWeight: 600, fontFamily: MONO, letterSpacing: -0.3 }}
                  >
                    {v}
                  </div>
            ...
            >
              <div style={{ fontSize: 13, color: t.text }}>
                {focusNode.dependents} services affected
              </div>
              <div style={{ fontSize: 11, color: t.textDim, marginTop: 2 }}>
                Includes 3 tier-1 customer flows
              </div>
            </div>
            ...
            {[
              ...
              >
                ...
                <span
                  style={{ marginLeft: 'auto', color: t.textDim, fontFamily: MONO, fontSize: 10 }}
                >
                  {prop}
                  {win ? '' : ' (lost)'}
                </span>
            ...
            <div style={{ display: 'flex', gap: 6, marginTop: 18 }}>
              <Btn
                ...
                onClick={() => nav('entity', focus)}
              >
                Open details →
              </Btn>
              <Btn small onClick={() => onAsk(`who would be impacted if ${focus} goes down?`)}>
                Ask ✦
              </Btn>
            </div>
          </div>
...
function GraphCanvas({ nodes, edges, focus, layout, onFocus, onHover, hover, onOpen }) {
  ...
  const positions = React.useMemo(() => {
    ...
    if (layout === 'radial') {
      ...
      nodes.forEach((n) => {
        (rows[n.type] = rows[n.type] || []).push(n);
      });
      const rowOrder = [
        'Team',
        'LogicalService',
  ...
  const hoverEdges = hover
    ? new Set(edges.filter((e) => e[0] === hover || e[1] === hover).map((_, i) => i))
    : new Set();
  const hoverNeighbors = hover
    ? new Set(edges.filter((e) => e[0] === hover || e[1] === hover).flatMap((e) => [e[0], e[1]]))
    : null;
      ...
      {nodes.map((n) => {
        ...
        return (
          ...
          >
            ...
            {isFocus && (
              <circle r={r + 18} fill="none" stroke={color} strokeWidth="0.5" opacity="0.2" />
            )}
            ...
            <text
              ...
              fill={isFocus ? t.text : t.textMuted}
              fontFamily={MONO}
              textAnchor="middle"
            ...
            {isFocus && (
              <text y={r + 28} fontSize="9" fill={t.textDim} fontFamily={MONO} textAnchor="middle">
                {n.owner}
              </text>
            )}
          </g>
