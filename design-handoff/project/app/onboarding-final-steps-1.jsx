...
function StepWorkspace({ workspace, setWorkspace }) {
  ...
  return (
    <>
      ...
      <StepLede>
        This is your company's workspace — everyone joins here. You can change it later (but you
        probably won't).
      </StepLede>
      <div
        style={{
...
function StepConnect({ sources, setSources }) {
  ...
  return (
    <>
      ...
      <StepLede>
        Each source becomes a lens on your infra. We reconcile them into one graph — with full
        provenance, so you can always see who said what.
      </StepLede>
...
function StepBuild({ sources }) {
  ...
  return (
    <>
      ...
      <StepHeadline>{done ? 'Your graph is alive.' : 'Building your graph.'}</StepHeadline>
      <StepLede>
        {done ? (
          "Eight seconds. Your team couldn't have done this by hand in a month."
        ) : (
          <>
            Reading every source, resolving every claim.{' '}
            <span style={{ color: t.text }}>{cur.text}</span>…
          </>
        )}
      </StepLede>
        ...
        {logLines.slice(-5).map((line, i, a) => {
          ...
          return (
            <div key={line.at} className="fade-in" style={{ color }}>
              <span style={{ color: t.textDim, marginRight: 8 }}>{head}</span>
              {line.text}
            </div>
          );
        })}
