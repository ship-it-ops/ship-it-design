...
function PageHome({ nav, onAsk }) {
  ...
  return (
    <div
      className="fade-in"
      style={{ overflow: 'auto', height: '100%', padding: '28px 36px 40px' }}
    >
      {/* Hero */}
      <div
        style={{
      ...
      >
        <div>
          ...
          <div style={{ color: t.textMuted, marginTop: 8, fontSize: 14, maxWidth: 560 }}>
            247 services mapped across 7 sources. Your graph is fresh — two deployments rolled this
            hour.
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <Btn ghost>Export report</Btn>
          <Btn primary onClick={() => nav('graph')}>
            Open Graph →
          </Btn>
        </div>
      </div>
      ...
      <Card pad={0} style={{ marginBottom: 20, overflow: 'hidden' }}>
        ...
        >
          ...
          >
            What's the blast radius if <span style={{ color: t.accent }}>payments-api</span> goes
            down?
          </div>
      ...
      >
        {[
          ...
          <Card key={i}>
            ...
            <div style={{ fontSize: 30, fontWeight: 500, letterSpacing: -1, fontFamily: MONO }}>
              {s.n}
            </div>
          ...
          {ACTIVITY.slice(0, 6).map((x, i) => {
            ...
            return (
              <div
                ...
                style={{
                  display: 'grid',
                  gridTemplateColumns: '50px 58px 160px 1fr auto',
                  gap: 12,
                  alignItems: 'center',
                  padding: '10px 0',
              ...
              >
                ...
                <MonoText style={{ fontSize: 10, color: cmap[x.c], letterSpacing: 1 }}>
                  {x.k}
                </MonoText>
        ...
        <Card>
          ...
          {CONNECTORS.slice(0, 6).map((c, i) => (
            ...
            >
              ...
              <div style={{ color: t.textDim, fontSize: 10, fontFamily: MONO, flex: 1 }}>
                {c.items}
              </div>
              <div
                style={{
      ...
      <Card pad={0} style={{ overflow: 'hidden' }}>
        ...
        >
          ...
          >
            <span>
              <Dot color={t.accent} /> services
            </span>
            <span>
              <Dot color={t.purple} /> repos
            </span>
            <span>
              <Dot color={t.ok} /> deployments
            </span>
            <span>
              <Dot color={t.pink} /> teams
            </span>
          </div>
          <div style={{ marginLeft: 16 }}>
            <Btn small onClick={() => nav('graph')}>
              Explore →
            </Btn>
          </div>
        </div>
