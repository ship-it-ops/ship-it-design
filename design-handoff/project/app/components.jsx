...
function Pill({ children, color, style = {} }) {
  ...
  return (
    <span style={{
      ...
      borderRadius: 999, fontFamily: MONO, letterSpacing: 0.3,
      ...style,
    }}>{children}</span>
  );
}
...
function SectionLabel({ children, right, style = {} }) {
  ...
  return (
    <div style={{
      ...
      fontSize: 10, color: t.textDim, fontFamily: MONO,
...
function IconGlyph({ name, size = 14, color }) {
  ...
  const map = {
    home: '◱', graph: '⋈', ask: '✦', connectors: '⌁', schema: '≡',
    incident: '◎', activity: '↗', settings: '⚙', search: '⌕',
    close: '×', chevron: '›', arrow: '→', check: '✓',
    sparkle: '✦', node: '◆', ring: '◯', grid: '▦',
    pause: '⏸', play: '▶', tier1: '◆', tier2: '◇',
  };
  return <span style={{ fontSize: size, color, lineHeight: 1, display: 'inline-block' }}>{map[name] || name}</span>;
}
...
function AskHint({ style = {} }) {
  ...
  return (
    <span style={{
      padding: '2px 6px', fontSize: 10, fontFamily: MONO,
      background: t.panel2, border: `1px solid ${t.border}`,
      borderRadius: 4, color: t.textDim, ...style,
    }}>⌘K</span>
  );
}

Object.assign(window, { Btn, Card, Pill, Dot, SectionLabel, PropRow, TypeBadge, StatusDot, MonoText, IconGlyph, AskHint });
