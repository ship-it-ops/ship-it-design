...
function PageAsk({ nav, seed }) {
  ...
  const [conversation, setConversation] = React.useState(() => seed ? [{ role: 'user', text: seed }, makeAssistantReply(seed)] : []);
  ...
  return (
    <div style={{ display: 'flex', height: '100%' }}>
      ...
      <div style={{ width: 240, borderRight: `1px solid ${t.border}`, background: t.panel, padding: '18px 14px', overflow: 'auto', flexShrink: 0 }}>
        <Btn primary style={{ width: '100%', justifyContent: 'center', marginBottom: 16 }} onClick={() => setConversation([])}>
          + New query
        </Btn>

        <SectionLabel>Suggested</SectionLabel>
        {SAMPLE_QUESTIONS.map(q => (
          <div key={q} onClick={() => submit(q)} style={{
            padding: '8px 10px', fontSize: 12, color: t.textMuted,
            borderRadius: 6, cursor: 'pointer', marginBottom: 2,
            lineHeight: 1.4,
          }}>
            <span style={{ color: t.accent, marginRight: 6 }}>✦</span>{q}
          </div>
        ))}

        <SectionLabel style={{ marginTop: 22 }}>Recent</SectionLabel>
        {[
          'dependencies of auth-svc',
          'who owns user-service',
          'services in staging only',
        ].map(q => (
          <div key={q} style={{
            padding: '8px 10px', fontSize: 12, color: t.textMuted,
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        {conversation.length === 0 ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 40 }}>
            <div style={{ fontSize: 11, color: t.textDim, fontFamily: MONO, textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 14 }}>Ask ShipIt</div>
            <div style={{ fontSize: 36, fontWeight: 500, letterSpacing: -0.8, textAlign: 'center', marginBottom: 10, maxWidth: 700 }}>
              Ask anything about your <span style={{ color: t.accent }}>software ecosystem.</span>
            </div>
            <div style={{ color: t.textMuted, fontSize: 14, maxWidth: 520, textAlign: 'center', marginBottom: 30 }}>
              Natural language queries over 247 services, 1,284 relationships, and 7 data sources. Uses the MCP server + Neo4j graph under the hood.
            </div>

            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center', maxWidth: 780 }}>
              {SAMPLE_QUESTIONS.map(q => (
                <div key={q} onClick={() => submit(q)} style={{
                  padding: '10px 14px', fontSize: 13, border: `1px solid ${t.border}`,
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: '28px 40px 20px' }}>
            {conversation.map((m, i) => m.role === 'user' ? (
              <div key={i} className="fade-in" style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 10, color: t.textDim, fontFamily: MONO, textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 6 }}>You asked</div>
                <div style={{ fontSize: 22, fontWeight: 500, letterSpacing: -0.4, lineHeight: 1.3 }}>{m.text}</div>
              </div>
            ) : (
              <AssistantReply key={i} msg={m} nav={nav}/>
            ))}
            {thinking && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: t.textMuted, fontSize: 12, padding: '12px 0' }}>
                <div style={{ width: 8, height: 8, borderRadius: 999, background: t.accent, animation: 'pulse 1s infinite' }}/>
                Traversing graph…
              </div>
            )}
          </div>
        ...
        <div style={{ borderTop: `1px solid ${t.border}`, padding: 16, background: t.bg }}>
          <div style={{
            ...
            <Btn small primary onClick={() => submit(query)}>Ask</Btn>
          </div>
          <div style={{ textAlign: 'center', fontSize: 10, color: t.textDim, fontFamily: MONO, marginTop: 8 }}>
...
function AssistantReply({ msg, nav }) {
  ...
  return (
    <div className="fade-in" style={{ marginBottom: 28 }}>
      <div style={{ fontSize: 10, color: t.accent, fontFamily: MONO, textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
        <Dot color={t.accent} glow/> Shipit
      </div>

      <div style={{ fontSize: 15, lineHeight: 1.55, color: t.text, marginBottom: 18, maxWidth: 780 }}>
        {msg.lead}
      </div>
      ...
      {msg.kind === 'impact' && (
        <Card style={{ marginBottom: 14 }}>
          <SectionLabel right={<span style={{ color: t.warn, fontFamily: MONO }}>BLAST RADIUS</span>}>Impacted services</SectionLabel>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {msg.items.map(i => (
              <div key={i.name} onClick={() => nav('entity', i.name)} style={{
                display: 'flex', alignItems: 'center', gap: 10,
                padding: '10px 12px', background: t.panel2, border: `1px solid ${t.border}`, borderRadius: 6, cursor: 'pointer',
              }}>
        ...
        <Card style={{ marginBottom: 14 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 0 }}>
            {msg.facts.map(([k, v, c], i) => (
              <div key={k} style={{ padding: '4px 16px', borderLeft: i ? `1px solid ${t.border}` : 'none' }}>
                <div style={{ fontSize: 10, color: t.textDim, fontFamily: MONO, textTransform: 'uppercase', letterSpacing: 1.3, marginBottom: 4 }}>{k}</div>
        ...
        <Card style={{ marginBottom: 14 }}>
          {msg.items.map((it, i) => (
            <div key={it.name} style={{
              display: 'grid', gridTemplateColumns: '16px 180px 1fr auto', gap: 12, alignItems: 'center',
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
        {msg.follows.map(f => (
          <div key={f} style={{
            padding: '5px 10px', fontSize: 11, border: `1px solid ${t.border}`,
            borderRadius: 999, color: t.textMuted, background: t.panel, cursor: 'pointer',
          }}>→ {f}</div>
        ))}
      </div>

      {/* Provenance */}
      <div style={{ fontSize: 10, color: t.textDim, fontFamily: MONO, display: 'flex', alignItems: 'center', gap: 8 }}>
        <span>sourced from:</span>
        {msg.sources.map(s => <span key={s} style={{ background: t.panel2, border: `1px solid ${t.border}`, borderRadius: 4, padding: '2px 6px' }}>{s}</span>)}
      </div>
    </div>
  );
}
...
function makeAssistantReply(q) {
  ...
  if (lower.includes('blast') || lower.includes('impact') || lower.includes('goes down') || lower.includes('impacted')) {
    return {
      role: 'assistant',
