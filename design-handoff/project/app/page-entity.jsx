...
function PageEntity({ id, nav, onAsk }) {
  ...
  return (
    <div
      className="fade-in"
      style={{ overflow: 'auto', height: '100%', padding: '24px 32px 40px' }}
    >
      <div
        onClick={() => nav('graph', s.id)}
        style={{
          fontSize: 12,
          color: t.textDim,
          fontFamily: MONO,
      ...
      >
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            ...
            <Pill color={s.tier === 1 ? t.err : s.tier === 2 ? t.warn : t.textMuted}>
              Tier {s.tier}
            </Pill>
            {s.env.map((e) => (
              <Pill key={e}>{e}</Pill>
          ...
          <div style={{ fontSize: 36, fontWeight: 500, letterSpacing: -0.9, fontFamily: MONO }}>
            {s.name}
          </div>
          <div style={{ color: t.textMuted, marginTop: 6, fontSize: 14, maxWidth: 640 }}>
            {s.desc}
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <Btn onClick={() => nav('graph', s.id)}>Focus in graph</Btn>
          <Btn onClick={() => onAsk(`explain ${s.name} for me`)} primary>
            Ask about this ✦
          </Btn>
        </div>
      </div>
      ...
      <Card pad={0} style={{ marginBottom: 16 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)' }}>
          {[
            ...
            <div
              key={k}
              style={{ padding: '16px 20px', borderLeft: i ? `1px solid ${t.border}` : 'none' }}
            >
              <div
                style={{
                  fontSize: 10,
              ...
              <div
                style={{ fontSize: 18, fontWeight: 500, color: c || t.text, letterSpacing: -0.3 }}
              >
                {v}
              </div>
            </div>
      ...
      >
        <SectionLabel>
          <Dot color={t.accent} glow /> <span style={{ marginLeft: 8 }}>Shipit · context</span>
        </SectionLabel>
        <div style={{ fontSize: 16, color: t.text, lineHeight: 1.5, marginBottom: 10 }}>
          {s.name} sits in the critical path for checkout. It depends on {s.deps} services and has{' '}
          {s.dependents} dependents. Last change was {s.change}.
        </div>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {['Blast radius', 'Who to page', 'Recent deploys', 'Show in graph'].map((a) => (
            <div
              key={a}
              onClick={() => onAsk(`${a.toLowerCase()} for ${s.name}`)}
              style={{
                padding: '6px 11px',
                fontSize: 12,
      ...
      <Card>
        ...
        <div
          style={{
            display: 'grid',
            gridTemplateColumns: '120px 120px 1fr 80px 80px',
            gap: 12,
            fontSize: 10,
            color: t.textDim,
            fontFamily: MONO,
