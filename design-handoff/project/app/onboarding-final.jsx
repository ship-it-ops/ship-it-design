...
function OnboardingFinal({ persona = 'admin', onFinish }) {
  ...
  const [sources, setSources] = React.useState({
    github: true, k8s: true, datadog: true, backstage: false, pagerduty: false,
  });
  ...
  const go = (d) => {
    ...
    if (next === STEPS.length) { onFinish && onFinish(); return; }
    setDir(d);
    setStep(next);
  };
  ...
  return (
    <div style={{
      ...
      {/* Top chrome */}
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, padding: '22px 28px', display: 'flex', alignItems: 'center', zIndex: 2 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 24, height: 24, borderRadius: 7, background: `linear-gradient(135deg, ${t.accent}, ${t.purple})`, display: 'grid', placeItems: 'center', fontSize: 12, fontWeight: 700, color: '#0a0a0b' }}>◆</div>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 18 }}>
          <div style={{ fontSize: 11, color: t.textDim, fontFamily: MONO, letterSpacing: 0.3 }}>
            {String(step + 1).padStart(2, '0')}<span style={{ opacity: 0.4 }}> / {String(STEPS.length).padStart(2, '0')}</span>
          </div>
          <div onClick={() => onFinish && onFinish()} style={{ fontSize: 11, color: t.textDim, fontFamily: MONO, cursor: 'pointer' }}>skip setup →</div>
        </div>
      </div>
        ...
        <div style={{
          ...
          <div style={{ position: 'relative', minHeight: 360 }}>
            <StepTransition stepKey={cur} dir={dir}>
              {cur === 'workspace' && <StepWorkspace workspace={workspace} setWorkspace={setWorkspace}/>}
          ...
          {/* Footer */}
          <StepFooter
            step={step} total={STEPS.length} cur={cur}
            sources={sources} sCount={sCount} workspace={workspace}
            onBack={() => go(-1)} onNext={() => go(1)} onFinish={() => onFinish && onFinish()}
          />
        </div>

        {/* Decorative caption below card */}
        <div style={{ marginTop: 18, textAlign: 'center', fontSize: 11, color: t.textDim, fontFamily: MONO, letterSpacing: 0.5 }}>
          {footerCaption(cur)}
        </div>
      </div>
    </div>
...
function StepFooter({ step, total, cur, sources, sCount, workspace, onBack, onNext, onFinish }) {
  ...
  const disabled =
    (cur === 'connect' && sCount === 0) ||
    (cur === 'workspace' && !workspace.trim());
  const primaryLabel =
    cur === 'build' ? 'Continue →' :
    isLast ? 'Open ShipIt →' :
    cur === 'workspace' ? 'Set up workspace →' :
    cur === 'connect' ? `Build graph · ${sCount} source${sCount === 1 ? '' : 's'} →` :
    cur === 'invite' ? 'Send invites →' :
    'Continue →';
  ...
  return (
    <div style={{
      ...
      <div>
        {step > 0 ? (
          <span onClick={onBack} style={{ fontSize: 11, color: t.textMuted, fontFamily: MONO, cursor: 'pointer', padding: '6px 10px', borderRadius: 5 }}>
            ← back
          </span>
        ) : <span style={{ fontSize: 11, color: t.textDim, fontFamily: MONO, padding: '6px 10px' }}>⌘⌥1 · setup</span>}
      </div>
      <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
        {!isLast && cur !== 'build' && cur !== 'connect' && cur !== 'workspace' && (
          <Btn small ghost onClick={onNext}>Skip</Btn>
        )}
        <Btn small primary onClick={isLast ? onFinish : onNext} style={disabled ? { opacity: 0.4, pointerEvents: 'none' } : {}}>
          {primaryLabel}
        </Btn>
      </div>
    </div>
...
function StepTransition({ children, stepKey, dir }) {
  ...
  return (
    <div style={{ position: 'relative', padding: '34px 40px 32px' }}>
      {prev && (
        <div key={prev.key + '-out'} className="step-out" style={{ position: 'absolute', inset: '34px 40px 32px', animationDirection: prev.dir > 0 ? 'normal' : 'reverse' }}>
          {prev.children}
        </div>
      )}
