...
function OnboardingVariantD({ persona = 'admin' }) {
  ...
  const [sources, setSources] = React.useState({ github: true, k8s: true, datadog: true, backstage: true, pagerduty: false });
  ...
  const allMessages = [
    ...
    { at: 'connect', who: 'shipit', text: "Plug in some sources. The more you give me, the more I can show you." },
    { at: 'schema', who: 'shipit', text: "Here's the unified schema I'll use. Any conflicts get tracked as PropertyClaims." },
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', background: t.bg, color: t.text, fontFamily: FONT, overflow: 'hidden' }}>
      {/* Left: conversational rail */}
      <div style={{ width: 360, padding: '22px 24px', borderRight: `1px solid ${t.border}`, background: t.panel, display: 'flex', flexDirection: 'column', flexShrink: 0, overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
          {visibleMessages.map((m, i) => (
            <div key={i} className="fade-in" style={{ display: 'flex', gap: 8 }}>
              <div style={{ width: 20, height: 20, borderRadius: 999, background: t.accentDim, color: t.accent, display: 'grid', placeItems: 'center', fontSize: 10, flexShrink: 0, marginTop: 2 }}>✦</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, color: t.text, lineHeight: 1.4 }}>{m.text}</div>

                {/* Inline interactions per step */}
                {m.at === cur && cur === 'workspace' && (
                  <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', background: t.panel2, border: `1px solid ${t.borderStrong}`, borderRadius: 6, padding: '8px 10px' }}>
                    <input value={workspace} onChange={e => setWorkspace(e.target.value)} style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', color: t.text, fontSize: 13, fontFamily: FONT }}/>
                    <span style={{ color: t.textDim, fontSize: 11, fontFamily: MONO }}>.shipit.ai</span>
                  </div>
                )}

                {m.at === cur && cur === 'connect' && (
                  <div style={{ marginTop: 10, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {[['github','GitHub','⌨'],['k8s','K8s','⎔'],['datadog','DD','◉'],['backstage','Backstage','▨'],['pagerduty','PD','◔']].map(([k, n, ic]) => {
                      const on = sources[k];
                      return (
                        <div key={k} onClick={() => setSources({ ...sources, [k]: !on })} style={{
                          padding: '6px 10px', fontSize: 11, borderRadius: 999, cursor: 'pointer',
                          background: on ? t.accent : t.panel2, color: on ? '#0a0a0b' : t.textMuted,
                ...
                {m.at === cur && cur === 'sync' && (
                  <div style={{ marginTop: 10 }}>
                    ...
                    <div style={{ marginTop: 6, fontFamily: MONO, fontSize: 10, color: t.textDim }}>{Math.floor(pct)}% · {Math.floor(pct * 2.47)} services · {Math.floor(pct * 12.84)} edges</div>
                  </div>
                )}

                {m.at === cur && cur === 'ask' && (
                  <div style={{ marginTop: 10, padding: '8px 10px', background: t.panel2, border: `1px solid ${t.borderStrong}`, borderRadius: 6, fontSize: 12 }}>
                ...
                {m.at === cur && cur === 'aha' && (
                  <div style={{ marginTop: 10 }}>
                    {[
                      ...
                      <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0', fontSize: 11 }}>
                        <Dot color={c}/>{s}
                      </div>
                    ))}
                  </div>
                ...
                {m.at === cur && cur === 'invite' && (
                  <div style={{ marginTop: 10 }}>
                    <input placeholder="teammate@acme.com" style={{ width: '100%', padding: '8px 10px', background: t.panel2, border: `1px solid ${t.borderStrong}`, borderRadius: 6, color: t.text, fontSize: 12, fontFamily: FONT, outline: 'none', boxSizing: 'border-box' }}/>
                  </div>
                )}
              </div>
            </div>
        ...
        <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${t.border}`, display: 'flex', gap: 6 }}>
          {step > 0 && <Btn small onClick={() => setStep(step - 1)}>←</Btn>}
      ...
      <div style={{ flex: 1, position: 'relative', minWidth: 0, background: `radial-gradient(ellipse at 50% 50%, ${t.panel} 0%, ${t.bg} 80%)` }}>
        {/* Stat overlay */}
        <div style={{ position: 'absolute', top: 18, left: 22, display: 'flex', gap: 18, fontFamily: MONO, fontSize: 11, color: t.textMuted, zIndex: 2 }}>
          <span>workspace=<span style={{ color: t.accent }}>{workspace}</span></span>
          <span>sources=<span style={{ color: t.accent }}>{sCount}</span></span>
          <span>chapter=<span style={{ color: t.accent }}>{step + 1}/{STEPS.length}</span></span>
        </div>

        <div style={{ position: 'absolute', top: 18, right: 22, fontFamily: MONO, fontSize: 10, color: t.textDim, display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        {/* Bottom log */}
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '14px 22px', borderTop: `1px solid ${t.border}`, background: `${t.bg}cc`, backdropFilter: 'blur(6px)', fontFamily: MONO, fontSize: 11, color: t.textMuted, lineHeight: 1.7, maxHeight: 130, overflow: 'hidden' }}>
...
function CanvasGraphBuild({ step, pct, sourceCount }) {
  const t = useTheme();
  const reveal = step === 0 ? 0
    : step === 1 ? Math.min(0.25, sourceCount * 0.05)
    : step === 2 ? 0.4
    : step === 3 ? 0.4 + (pct / 100) * 0.6
    : 1;
  const all = SERVICES;
  ...
  return (
    <svg width="100%" height="100%" viewBox="0 0 760 480" preserveAspectRatio="xMidYMid meet" style={{ position: 'absolute', inset: 0 }}>
      <defs>
        <radialGradient id="dGlow"><stop offset="0" stopColor={t.accent} stopOpacity="0.3"/><stop offset="1" stopColor={t.accent} stopOpacity="0"/></radialGradient>
...
function BuildLog({ step, pct }) {
  ...
  if (step >= 3) {
    lines.push(`→ ingesting · ${Math.floor(pct)}% · ${Math.floor(pct * 2.47)} services · ${Math.floor(pct * 38.9)} claims`);
    ...
    <>
      {lines.slice(-6).map((l, i, a) => (
        <div key={i} style={{ color: i === a.length - 1 ? t.text : t.textMuted }}>
          <span style={{ color: t.textDim, marginRight: 6 }}>{String(i).padStart(2, '0')}</span>{l}
        </div>
      ))}
    </>
  );
